...
import { CubeIcon, PlusCircleIcon, TrashIcon, PencilSquareIcon, ArrowLeftCircleIcon } from '@heroicons/react/24/solid';
...
export default function AdminModelos() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center py-8">
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-blue-100 p-8 animate-fade-in">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <CubeIcon className="h-8 w-8 text-blue-500 ml-4" />
            <h1 className="text-3xl font-extrabold text-blue-900 tracking-tight">Gestionar Modelos</h1>
          </div>
          <div className="flex gap-4">
            ...
            >
              <PlusCircleIcon className="h-6 w-6" /> Nuevo Modelo
            </Link>
            <button
          ...
          <table className="min-w-full divide-y divide-blue-200">
            <thead className="bg-blue-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Modelo</th>
                <th className="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Marca</th>
                <th className="px-6 py-3 text-right text-xs font-bold text-blue-700 uppercase tracking-wider">Acciones</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-blue-100">
              {modelos.map((modelo) => (
                <tr key={modelo.id} className="hover:bg-blue-50 transition-all">
                  <td className="px-6 py-4 whitespace-nowrap text-base font-bold text-blue-900">{modelo.id}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-blue-900">{modelo.nombre}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-blue-900">{getMarcaNombre(modelo.marca_id)}</td>
        ...
        {modelos.length === 0 && (
          <div className="text-center py-12">
            <p className="text-blue-500">No hay modelos registrados aún.</p>
            <Link
              href="/admin/modelos/nuevo"
            ...
            >
              <PlusCircleIcon className="h-6 w-6 mr-2" /> Agregar el primer modelo
            </Link>
          </div>
        )}
      </div>
