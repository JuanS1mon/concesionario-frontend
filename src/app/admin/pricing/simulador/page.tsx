...
function PricingSimuladorContent() {
  ...
  const [updateMessage, setUpdateMessage] = useState<{ msg: string; ok: boolean } | null>(null);
  ...
  const handleUpdatePrice = useCallback(async () => {
    ...
    try {
      await pricingAPI.actualizarPrecio(selectedAutoId, precioSlider);
      setUpdateMessage({ msg: 'Precio actualizado correctamente', ok: true });
      // Recargar análisis para reflejar el cambio
      loadAutoAnalisis(selectedAutoId);
      setTimeout(() => setUpdateMessage(null), 5000);
    } catch (error: any) {
      console.error('Error actualizando precio:', error);
      setUpdateMessage({ msg: 'Error al actualizar precio', ok: false });
      setTimeout(() => setUpdateMessage(null), 5000);
    } finally {
      setUpdatingPrice(false);
    }
  }, [selectedAutoId, precioSlider]);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow p-6">
              ...
              <div className="mb-6">
                ...
                {/* Botón para confirmar cambio de precio */}
                <div className="mt-4 flex items-center justify-between">
                  <div>
                    {updateMessage && (
                      <span className={`text-sm ${updateMessage.ok ? 'text-green-600' : 'text-red-600'}`}>
                        {updateMessage.msg}
                      </span>
                    )}
                  </div>
                  <button
                    ...
                    disabled={updatingPrice || precioSlider === analisis.precio_actual}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {updatingPrice ? 'Actualizando...' : 'Aplicar Precio'}
                  </button>
                </div>
              </div>
